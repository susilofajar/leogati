...
@section('header_title', 'Edit Kupon Promo: ' . $kupon->code)
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-xs space-y-6">
        <h2 class="text-lg font-bold text-slate-800 border-b border-slate-100 pb-3">Edit Kupon Promo: <span class="font-mono text-blue-600">{{ $kupon->code }}</span></h2>

        <form action="{{ route('admin.kupon.update', $kupon->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="code" id="code" value="{{ old('code', $kupon->code) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $kupon->name) }}" required
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        <option value="percent" {{ old('type', $kupon->type) === 'percent' ? 'selected' : '' }}>Persentase Diskon (%)</option>
                        <option value="fixed" {{ old('type', $kupon->type) === 'fixed' ? 'selected' : '' }}>Potongan Nominal Tetap (Rp)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="value" id="value" value="{{ old('value', $kupon->value) }}" step="0.01" required
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="min_purchase_amount" id="min_purchase_amount" value="{{ old('min_purchase_amount', $kupon->min_purchase_amount) }}" step="1"
                        class="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs focus:outline-hidden focus:border-[#0B5CFF]">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="max_discount_amount" id="max_discount_amount" value="{{ old('max_discount_amount', $kupon->max_discount_amount) }}" step="1"
                        class="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs focus:outline-hidden focus:border-[#0B5CFF]">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" name="usage_limit" id="usage_limit" value="{{ old('usage_limit', $kupon->usage_limit) }}" step="1"
                        class="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs focus:outline-hidden focus:border-[#0B5CFF]">
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="start_date" id="start_date" 
                        value="{{ old('start_date', $kupon->start_date ? $kupon->start_date->format('Y-m-d\TH:i') : '') }}"
                        class="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs focus:outline-hidden focus:border-[#0B5CFF]">
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="end_date" id="end_date" 
                        value="{{ old('end_date', $kupon->end_date ? $kupon->end_date->format('Y-m-d\TH:i') : '') }}"
                        class="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs focus:outline-hidden focus:border-[#0B5CFF]">
                </div>
            </div>
            ...
            <div class="pt-2">
                <label class="flex items-center space-x-2 text-xs font-bold text-slate-700 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $kupon->is_active) ? 'checked' : '' }}
                        class="w-4 h-4 text-blue-600 rounded border-slate-300">
                    <span>Aktifkan kupon promo ini</span>
                </label>
            </div>
