...
@section('header_title', 'Manajemen Merek Resmi')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-xl font-extrabold text-slate-900">Merek Resmi Mitra</h2>
            <p class="text-xs text-slate-500 mt-1">Daftar merek resmi produsen hardware dan aksesoris komputer mitra LEOGATISTORE.</p>
        </div>
        <a href="{{ route('admin.merek.create') }}" 
    ...
    <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs">
        <form action="{{ route('admin.merek.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3">
            <div class="flex-1 relative">
                <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari nama atau slug merek..."
            ...
            @if(request()->hasAny(['q', 'status']))
                <a href="{{ route('admin.merek.index') }}" class="px-3 py-2 bg-slate-100 text-slate-600 font-bold text-xs rounded-xl hover:bg-slate-200 transition flex items-center justify-center">
                    Reset
                </a>
            @endif
        </form>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-xs text-left">
                <thead class="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase tracking-wider font-bold">
                    <tr>
                        <th class="px-5 py-3.5">Nama Merek</th>
                        ...
                        <tr class="hover:bg-slate-50/80 transition">
                            ...
                            <td class="px-5 py-4 text-right">
                                <div class="flex items-center justify-end space-x-2">
                                    <a href="{{ route('admin.merek.edit', $brand->id) }}" 
                                        class="px-3 py-1.5 bg-slate-100 hover:bg-[#0B5CFF] text-slate-700 hover:text-white font-bold rounded-lg transition">
                                        Ubah
                                    </a>
                                    <form action="{{ route('admin.merek.destroy', $brand->id) }}" method="POST" class="inline"
                                        onsubmit="return confirm('Hapus merek ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-8 text-center text-slate-500">
                                Tidak ada merek yang ditemukan.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
